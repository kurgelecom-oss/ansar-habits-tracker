...
/* Pulls Ansar's season from Football Victoria's Dribl Match Centre into
   ...
   Config: app/pathway/data/team.json → { tenant, club, team }.
...
try {
  ...
  const result = await page.evaluate(async ({ tenantSlug, clubName, teamFilter }) => {
    ...
    const mine = rows.filter(r => {
      ...
      const side = ours(a.home_team_name) ? a.home_team_name : a.away_team_name;
      return !filter || (side ?? "").toLowerCase().includes(filter);
    });
    return { season: season.name, club: club.attributes.name, clubLogo: club.attributes.image ?? null, teams, fixtures: mine.map(r => {
      const a = r.attributes;
      return {
        id: r.hash_id, date: a.date, round: a.full_round ?? a.round, competition: a.competition_name, league: a.league_name,
  ...
  }, { tenantSlug: cfg.tenant, clubName: cfg.club, teamFilter: cfg.team });

  if (result.error) throw new Error(result.error);
  result.fixtures.sort((x, y) => x.date.localeCompare(y.date));
  const out = { configured: true, club: result.club, clubLogo: result.clubLogo, team: cfg.team || null, season: result.season, syncedAt: new Date().toISOString(), teams: result.teams, fixtures: result.fixtures, source: `https://${cfg.tenant}.dribl.com/fixtures/` };
  await writeFile(OUT, JSON.stringify(out, null, 1) + "\n");
  console.log(`${result.club} · season ${result.season} · ${result.fixtures.length} fixtures · ${result.teams.length} club teams`);
} finally {
  await browser.close();
}
