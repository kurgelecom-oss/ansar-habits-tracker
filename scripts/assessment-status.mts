...
const [lessons,papers,state,outbox]=await Promise.all([
...
for(const response of [lessons,papers,state,outbox])if(response.error)throw new Error('Unable to read assessment status. Check the private connection and deployed schema.');
...
const attempts=learnerPaperIds.size ? await db.from('ansar_assessment_attempts').select('paper_id,status').in('paper_id',[...learnerPaperIds]) : {data:[],error:null};
if(attempts.error)throw new Error('Unable to read learner attempt counts.');
const learnerAttempts=attempts.data||[];
const subjects=[...new Set((lessons.data||[]).map(l=>l.subject))].sort();
console.log(JSON.stringify({
 month,today,completedSyncAt:state.data?.updated_at||null,
 warnings:state.data?.payload?.warnings||[],pendingDeliveriesAllModes:outbox.count||0,
 ...
 note:'Read-only status. Eligible dates do not prove teaching or mastery. Practice excluded from learner papers/attempt counts; pending deliveries include all modes. A completed sync timestamp must be newer than the refresh you requested.',
},null,2));
