    ...
    <div className={styles.content}>
      ...
      {locked ? <section className={styles.unlock}><p className={styles.eyebrow}>{practice ? "ISOLATED PARENT PRACTICE" : "A PRIVATE SPACE TO LEARN"}</p><h2>{practice ? "Parent, unlock practice." : "Parent, unlock this device."}</h2><p>{practice ? "Your parent PIN opens a separate practice session. Learner device access alone cannot open this room." : "Use your existing parent PIN once, then Ansar can work on this device. Publishing papers and reviewing answers still need your PIN."}</p><form onSubmit={unlock}><PinField value={pin} onChange={setPin} /><button className={styles.primary} disabled={busy}>{busy ? "Unlocking…" : practice ? "Unlock practice room" : "Unlock assessment room"}</button></form></section> : <>
        ...
        {workspace && <>
          ...
          {practice ? <PracticeTools activeAttempt={attempt} onAttempt={updateAttempt} onComplete={async text => { setNotice(text); await load(); }} /> : parentOpen && !inProgress && <ParentSync integrations={workspace.integrations} onComplete={async text => { setNotice(text); await load(); }} />}
...
function PracticeTools({ activeAttempt, onAttempt, onComplete }: { activeAttempt?: Attempt; onAttempt: (attempt: Attempt) => void; onComplete: (text: string) => Promise<void> }) {
  const mutate = useMutate(); const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  async function create(kind: "exam" | "review") { setBusy(true); setError(""); try { await mutate({ action: "create", kind }); await onComplete(`Fresh practice ${kind} created. Originals remain available.`); } catch (err) { setError(message(err)); } finally { setBusy(false); } }
  ...
  return <section className={`${styles.parentPanel} ${styles.practiceTools}`}><h3>Practice controls</h3><p>Create fresh fictional copies as often as needed. Existing practice papers remain intact.</p><div className={styles.practiceActions}><button disabled={busy} onClick={() => void create("exam")}>Create fresh 10-minute exam</button><button disabled={busy} onClick={() => void create("review")}>Create fresh 4-prompt review</button>{activeAttempt?.status === "in_progress" && activeAttempt.paper_snapshot.kind === "exam" && <button className={styles.expireButton} disabled={busy} onClick={() => void expire()}>Expire practice timer now</button>}</div>{error && <p role="alert" className={styles.error}>{error}</p>}</section>;
}
...
function SubmittedWork({ attempt, onAttempt, practice = false }: { attempt: Attempt; onAttempt: (attempt: Attempt) => void; practice?: boolean }) {
  ...
  return <div className={styles.submitted}><div className={styles.feedbackHeader}><span className={styles.badge}>{attempt.status === "reviewed" ? "Reviewed by Nihal" : "Submitted · awaiting Nihal"}</span><h3>{attempt.result?.summary || "Your work is safely submitted."}</h3>{attempt.result && <p>{attempt.result.objectiveTotal > 0 && `${attempt.result.objectiveCorrect}/${attempt.result.objectiveTotal} multiple choice correct · `}{attempt.result.writtenPending ? `${attempt.result.writtenPending} written responses awaiting review` : `${attempt.result.writtenPoints}/${attempt.result.writtenTotal} written points`}</p>}<small>Your original answers stay unchanged. Completion does not mean mastery.</small></div>
    ...
    {practice && <PracticeReport attempt={attempt} />}
...
function PracticeReport({ attempt }: { attempt: Attempt }) {
  ...
  return <section className={`${styles.parentPanel} ${styles.reportPanel}`}><h3>Practice report output</h3><p>Preview stays private. Nothing is sent until you choose the labelled send action.</p>{!report ? <button disabled={busy} onClick={() => void act("report-preview")}>{busy ? "Preparing preview…" : "Preview practice report"}</button> : <><h4>{report.subject}</h4><pre>{report.text}</pre><button className={styles.primary} disabled={busy} onClick={() => void act("send-report")}>{busy ? "Sending…" : "Send labelled practice report"}</button></>}{notice && <p role="status" className={styles.notice}>{notice}</p>}{error && <p role="alert" className={styles.error}>{error}</p>}</section>;
}
