...
type MutationResult = { attempt?: Attempt; paper?: Paper; previewVersion?: string; message?: string; report?: { subject: string; text: string; report: unknown } };
const MutationContext = createContext<(body: unknown) => Promise<MutationResult>>(() => Promise.reject(new Error("Assessment workspace is unavailable.")));
const useMutate = () => useContext(MutationContext);
...
export default function TestsPage() { return <AssessmentWorkspace />; }

export function AssessmentWorkspace({ practice = false }: { practice?: boolean }) {
  const apiPath = practice ? "/api/assessments/practice" : "/api/assessments";
  const sessionPath = `${apiPath}/session`;
  const mutate = useMemo(() => (body: unknown) => request<MutationResult>(apiPath, body), [apiPath]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await request<Workspace>(`${apiPath}?month=${month}`);
      ...
      setSelected(current => {
        ...
        if (desired && data.papers.some(p => p.id === desired)) { desiredSelection.current = ""; return desired; }
        return data.attempts.find(a => a.status === "in_progress")?.paper_id || (data.papers.some(p => p.id === current) ? current : preferredPaperId(data));
      });
    } catch (err) {
      if (id !== loadId.current) return;
  ...
  async function unlock(event: FormEvent) {
    ...
    try { await request(sessionPath, { pin }); setPin(""); await load(); }
    catch (err) { setError(message(err)); } finally { setBusy(false); }
  }
  async function logout() {
    setBusy(true); setError("");
    try { await request(sessionPath, undefined, "DELETE"); setWorkspace(null); setLocked(true); setParentOpen(false); }
    catch (err) { setError(message(err)); } finally { setBusy(false); }
  }
  ...
  async function selectCreatedPaper(created: Paper) {
    desiredSelection.current = created.id;
    setNotice(`Fresh practice ${created.kind} created. Originals remain available.`);
    if (created.month === month) await load();
    else setMonth(created.month);
  }
  return <MutationContext.Provider value={mutate}><main className={`${styles.page} ${practice ? styles.practicePage : ""}`} aria-label={practice ? "ANSAR OS Parent Practice" : "ANSAR OS Tests"}>
    <ClubNavigation activeLabel="Tests" />
    <div className={styles.content}>
      <header className={styles.hero}><div><p className={styles.eyebrow}>{practice ? "ANSAR OS · PARENT PRACTICE ROOM" : "ANSAR OS · ASSESSMENT ROOM"}</p><h1>{practice ? "Rehearse the whole workflow." : "Show what stayed."}</h1><p className={styles.lead}>{practice ? "Use fictional samples to test every step without changing the learner’s record." : "Recall the lesson. Explain your thinking. Build on the gaps."}</p></div><span className={styles.heroMark} aria-hidden="true">A<span>{practice ? "PRACTICE ONLY" : "LEARNING RECORD"}</span></span></header>
      {practice && <aside className={styles.practiceBanner}><strong>PARENT PRACTICE · DOES NOT AFFECT THE LEARNING RECORD</strong><span>Sample papers, answers, scores and reports stay isolated from live assessments and reminders.</span><a href="/tests">Back to live assessments</a></aside>}
      {error && <div role="alert" className={styles.error}>{error} {!locked && <button onClick={() => void load()}>Try again</button>}</div>}
      {notice && <p role="status" className={styles.notice}>{notice}</p>}
      {locked ? <section className={styles.unlock}><p className={styles.eyebrow}>{practice ? "ISOLATED PARENT PRACTICE" : "A PRIVATE SPACE TO LEARN"}</p><h2>{practice ? "Parent, unlock practice." : "Parent, unlock this device."}</h2><p>{practice ? "Your parent PIN opens a separate practice session. Learner device access alone cannot open this room." : "Use your existing parent PIN once, then Ansar can work on this device. Publishing papers and reviewing answers still need your PIN."}</p><form onSubmit={unlock}><PinField value={pin} onChange={setPin} /><button className={styles.primary} disabled={busy}>{busy ? "Unlocking…" : practice ? "Unlock practice room" : "Unlock assessment room"}</button></form></section> : <>
        <div className={styles.toolbar}><div className={styles.monthNav}><button aria-label="Previous month" disabled={loading || inProgress} onClick={() => setMonth(m => shiftMonth(m, -1))}>←</button><h2>{practice && "Practice · "}{new Date(`${month}-15T12:00:00Z`).toLocaleDateString("en-AU", { month: "long", year: "numeric" })}</h2><button aria-label="Next month" disabled={loading || inProgress} onClick={() => setMonth(m => shiftMonth(m, 1))}>→</button></div>{workspace && <div className={styles.actions}>{!practice && <><a className={styles.practiceLink} href="/tests/practice">Parent practice room</a><button disabled={inProgress} onClick={() => setParentOpen(v => !v)} aria-expanded={parentOpen}>Parent tools</button></>}<button disabled={busy || inProgress} onClick={() => void logout()}>Lock device</button></div>}</div>
        ...
        {workspace && <>
          ...
          <div className={styles.room}>
            <aside className={styles.assignments} aria-label="Assessments"><div className={styles.sectionHeading}><h2>Your work</h2><span>{workspace.papers.length} papers</span></div><div className={styles.filters} aria-label="Assessment type">{[["all", "All"], ["review", "Friday recall"], ["exam", "Monthly exam"]].map(([value, label]) => <button key={value} aria-pressed={filter === value} disabled={inProgress} onClick={() => changeFilter(value)}>{label}</button>)}</div>
              {papers.length === 0 && <p className={styles.empty}>{practice ? "No practice papers yet. Create a fresh sample above to add a practice paper." : <>No {filter === "all" ? "assessments" : filter === "exam" ? "monthly exams" : "Friday recalls"} yet. Papers appear here when programme coverage is available.</>}</p>}
            ...
            <section className={styles.station} aria-label="Selected assessment">
              {paper ? <><div className={styles.stationHeader}><p className={styles.eyebrow}>{paper.kind === "exam" ? "MONTHLY EXAM" : "FRIDAY RECALL"}</p><h2>{paper.subject}</h2><p>{paper.title}</p><div className={styles.metadata}><span>{isHistoricalBaseline(paper) ? "Coverage through" : "Due"} {dateLabel(paper.due_date)}</span><span>{paper.kind === "exam" && !attempt ? 12 : paper.questions.length} questions</span><span>{paper.kind === "exam" ? `${paper.duration_minutes || 25} minutes` : "Take your time"}</span></div></div>
                ...
                {attempt && attempt.status !== "in_progress" && <SubmittedWork key={attempt.id} attempt={attempt} onAttempt={updateAttempt} practice={practice} />}
              </> : <div className={styles.empty}><h2>{practice ? "A clean practice room." : "A fresh learning record."}</h2><p>{practice ? "Create a fresh sample exam or review above to begin a complete rehearsal." : "Your Friday recalls and monthly exams will appear after the programme is synced. Nihal can refresh the curriculum in Parent tools."}</p></div>}
            </section>
          </div>
...
function PracticeTools({ activeAttempt, onAttempt, onCreated }: { activeAttempt?: Attempt; onAttempt: (attempt: Attempt) => void; onCreated: (paper: Paper) => Promise<void> }) {
  const mutate = useMutate(); const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  async function create(kind: "exam" | "review") { setBusy(true); setError(""); try { const result = await mutate({ action: "create", kind }); if (!result.paper) throw new Error("The fresh practice paper was not returned."); await onCreated(result.paper); } catch (err) { setError(message(err)); } finally { setBusy(false); } }
  async function expire() { if (!activeAttempt) return; setBusy(true); setError(""); try { const result = await mutate({ action: "expire", attemptId: activeAttempt.id }); if (!result.attempt) throw new Error("Practice expiry was not confirmed."); onAttempt(result.attempt); } catch (err) { setError(message(err)); } finally { setBusy(false); } }
  const inProgress = activeAttempt?.status === "in_progress";
  return <section className={`${styles.parentPanel} ${styles.practiceTools}`}><h3>Practice controls</h3><p>Create fresh fictional copies as often as needed. Existing practice papers remain intact.</p><div className={styles.practiceActions}><button disabled={busy || inProgress} onClick={() => void create("exam")}>Create fresh 10-minute exam</button><button disabled={busy || inProgress} onClick={() => void create("review")}>Create fresh 4-prompt review</button>{inProgress && activeAttempt.paper_snapshot.kind === "exam" && <button className={styles.expireButton} disabled={busy} onClick={() => void expire()}>Expire practice timer now</button>}</div>{inProgress && <p className={styles.muted}>Finish or expire this attempt before creating another sample so saved work stays selected.</p>}{error && <p role="alert" className={styles.error}>{error}</p>}</section>;
}
...
function SubmittedWork({ attempt, onAttempt, practice = false }: { attempt: Attempt; onAttempt: (attempt: Attempt) => void; practice?: boolean }) {
  const mutate = useMutate();
  ...
  return <div className={styles.submitted}><div className={styles.feedbackHeader}><span className={styles.badge}>{attempt.status === "reviewed" ? "Reviewed by Nihal" : "Submitted · awaiting Nihal"}</span><h3>{attempt.result?.summary || "Your work is safely submitted."}</h3>{attempt.result && <p>{attempt.result.objectiveTotal > 0 && `${attempt.result.objectiveCorrect}/${attempt.result.objectiveTotal} multiple choice correct · `}{attempt.result.writtenPending ? `${attempt.result.writtenPending} written responses awaiting review` : `${attempt.result.writtenPoints}/${attempt.result.writtenTotal} written points`}</p>}<small>Your original answers stay unchanged. Completion does not mean mastery.</small></div>
    ...
    {practice && <PracticeReport key={`${attempt.id}:${attempt.revision}:${attempt.status}:${attempt.parent_review?.reviewedAt || ""}:${attempt.correction_at || ""}`} attempt={attempt} />}
...
function PracticeReport({ attempt }: { attempt: Attempt }) {
  const mutate = useMutate(); const [report, setReport] = useState<{ subject: string; text: string; report: unknown } | null>(null); const [notice, setNotice] = useState(""); const [error, setError] = useState(""); const [busy, setBusy] = useState(false);
  async function act(action: "report-preview" | "send-report") { setBusy(true); setError(""); setNotice(""); try { const result = await mutate({ action, attemptId: attempt.id }); if (action === "report-preview") { if (!result.report) throw new Error("Practice report preview was not returned."); setReport(result.report); } else setNotice(result.message || "Labelled practice report sent."); } catch (err) { setError(message(err)); } finally { setBusy(false); } }
  return <section className={`${styles.parentPanel} ${styles.reportPanel}`}><h3>Practice report output</h3><p>Preview stays private. Nothing is sent until you choose the labelled send action.</p>{!report ? <button disabled={busy} onClick={() => void act("report-preview")}>{busy ? "Preparing preview…" : "Preview practice report"}</button> : <><h4>{report.subject}</h4><h5>Short email preview</h5><pre>{report.text}</pre><h5>Full Notion report preview</h5><pre>{typeof report.report === "string" ? report.report : JSON.stringify(report.report, null, 2)}</pre><button className={styles.primary} disabled={busy} onClick={() => void act("send-report")}>{busy ? "Sending…" : "Send labelled practice report"}</button></>}{notice && <p role="status" className={styles.notice}>{notice}</p>}{error && <p role="alert" className={styles.error}>{error}</p>}</section>;
}
