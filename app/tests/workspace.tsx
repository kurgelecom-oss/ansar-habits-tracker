...
function PracticeReport({ attempt }: { attempt: Attempt }) {
  ...
  return <section className={`${styles.parentPanel} ${styles.reportPanel}`}><h3>Practice report output</h3><p>Preview stays private. Nothing is sent until you choose the labelled send action.</p>{!report ? <button disabled={busy} onClick={() => void act("report-preview")}>{busy ? "Preparing preview…" : "Preview practice report"}</button> : <><h4>{report.subject}</h4><h5>Short email preview</h5><pre>{report.text}</pre><h5>Full Notion report preview</h5><pre>{JSON.stringify(report.report, null, 2)}</pre><button className={styles.primary} disabled={busy} onClick={() => void act("send-report")}>{busy ? "Sending…" : "Send labelled practice report"}</button></>}{notice && <p role="status" className={styles.notice}>{notice}</p>}{error && <p role="alert" className={styles.error}>{error}</p>}</section>;
}
