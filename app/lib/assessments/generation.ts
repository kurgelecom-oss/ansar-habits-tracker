...
/** Balance eight answer positions without changing the keyed text or question content. */
export function balanceChoiceOptions(questions: Question[], seed: string): Question[] {
  if (questions.filter(q => q.type === 'choice').length !== 8) throw new Error('Answer balancing requires eight choice questions');
  const positions = [0, 0, 1, 1, 2, 2, 3, 3];
  for (let i = positions.length - 1; i > 0; i--) {
    const pick = createHash('sha256').update(`${seed}:answer-position:${i}`).digest().readUInt32BE(0) % (i + 1);
    [positions[i], positions[pick]] = [positions[pick], positions[i]];
  }
  let next = 0;
  return questions.map(question => {
    if (question.type !== 'choice') return { ...question };
    if (!question.options || question.options.length !== 4 || !Number.isInteger(question.answer) || question.answer! < 0 || question.answer! > 3) throw new Error('Cannot balance an invalid choice question');
    const options = [...question.options];
    const current = question.answer!;
    const answer = positions[next++];
    [options[answer], options[current]] = [options[current], options[answer]];
...
export async function generateExam(lessons: Lesson[], month: string): Promise<Paper> {
  ...
  return {
    ...
    questions: balanceChoiceOptions(validateExamQuestions(parsed.questions, assessableLessons), `exam:${month}:${subjectSlug(subject)}`), lessons, coverage_note: coverageNote(lessons),
  };
}
