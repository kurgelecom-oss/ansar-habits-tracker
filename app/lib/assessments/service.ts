...
export async function startPaper(id:unknown,scope:AssessmentScope='learner'){if(typeof id!=='string'||id.length>180)throw new AssessmentError('Invalid assessment.');const r=await adminClient().rpc('start_ansar_assessment_scoped',{p_paper_id:id,p_is_practice:scope==='practice'});if(r.error)throw new AssessmentError(/not open/.test(r.error.message)?'This assessment is not open yet.':/approval/.test(r.error.message)?'Nihal needs to approve this exam first.':'Unable to start this assessment.',409);return publicAttempt(r.data as Attempt);}
export async function saveAttempt(body:Record<string,unknown>,scope:AssessmentScope='learner'){
...
export async function publishPaper(body:Record<string,unknown>,scope:AssessmentScope='learner'){
 ...
 const preview=await previewPaper(body.paperId,scope);if(body.previewVersion!==preview.previewVersion)throw new AssessmentError('The curriculum changed after your preview. Review the new paper before approving.',409);
 let q=adminClient().from(PAPERS).update({status:'published',duration_minutes:duration,published_at:new Date().toISOString()}).eq('id',body.paperId).eq('kind','exam').eq('status','draft').eq('questions',JSON.stringify(preview.paper.questions)).eq('lessons',JSON.stringify(preview.paper.lessons));q=scope==='practice'?q.eq('is_practice',true):q.or('is_practice.eq.false,is_practice.is.null');const r=await q.select('*').maybeSingle();
...
export async function reviewAttempt(body:Record<string,unknown>,scope:AssessmentScope='learner'){
 const a=await getAttempt(body.attemptId,scope);if(a.status==='reviewed'){await report(a);return publicAttempt(a);}if(a.status!=='submitted')throw new AssessmentError('Only submitted work can be reviewed.',409);
 await report(a);
...
export async function correctAttempt(body:Record<string,unknown>,scope:AssessmentScope='learner'){
 const a=await getAttempt(body.attemptId,scope);if(a.status!=='reviewed')throw new AssessmentError('Wait for Nihal’s feedback before writing your correction.');
 await report(a);
...
export async function previewPaper(id:unknown,scope:AssessmentScope='learner'){
 if(typeof id!=='string'||id.length>180)throw new AssessmentError('Choose a paper.');
 let q=adminClient().from(PAPERS).select('*').eq('id',id);q=scope==='practice'?q.eq('is_practice',true):q.or('is_practice.eq.false,is_practice.is.null');const p=checked(await q.maybeSingle());
