...
const COOKIE='ansar_assessment_device';
...
export function signSession(expires:number){const value=Buffer.from(JSON.stringify({expires,id:randomUUID(),role:'learner'})).toString('base64url');return `${value}.${signature(value)}`;}
export function validSession(token:string|undefined,now=Date.now()){
  ...
  try {const [value,sig,...extra]=token.split('.');const expected=signature(value);if(extra.length||!sig||sig.length!==expected.length||!timingSafeEqual(Buffer.from(sig),Buffer.from(expected)))return false;
    const payload=JSON.parse(Buffer.from(value,'base64url').toString());return payload.role==='learner'&&Number.isFinite(payload.expires)&&payload.expires>now;
  }catch{return false;}
}
